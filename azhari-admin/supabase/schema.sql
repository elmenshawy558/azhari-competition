...
begin
  ...
  new.approval_status := 'APPROVED';  -- registration is auto-approved; admin can still reject afterward
...
as $$
begin
  -- Without this guard, the UPDATE below fires this same AFTER trigger
  -- again (Postgres re-fires statement-level triggers for every UPDATE,
  -- even ones issued from inside another trigger, even when 0 rows
  -- change) — causing infinite recursion. pg_trigger_depth() > 1 means
  -- we're already inside a nested firing of this trigger, so skip.
