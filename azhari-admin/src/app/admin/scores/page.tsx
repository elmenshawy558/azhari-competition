...
export default function AdminScoresPage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <h1 className="font-display text-2xl font-bold text-ink">إدارة النتائج</h1>

      <section className="card flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="font-bold">نشر النتائج</h2>
          <p className="text-sm text-ink-faint">عند التفعيل، يرى كل متسابق نتيجته فقط. عند الإيقاف، لا يرى أحد أي نتيجة.</p>
        </div>
        <button onClick={togglePublish} disabled={toggling || !settings}
          className={`btn-sm rounded-full px-5 py-2 font-bold ${settings?.results_published ? "bg-teal text-white" : "bg-gray-200 text-ink-soft"}`}>
      ...
      <section className="card">
        ...
        {importing && <p className="text-sm text-ink-faint mt-2">جاري الاستيراد...</p>}
        {importLog && <div className="card bg-teal/5 border-teal text-sm mt-3">{importLog}</div>}
      </section>

      <section className="card overflow-x-auto p-0">
        {loading ? (
          <p className="p-6 text-ink-faint">جاري التحميل...</p>
        ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b">
                  ...
                  <td className="p-3 text-ink-faint">{r.memorization_level}</td>
                  <td className="p-2">
              ...
              {rows.length === 0 && (
                <tr><td colSpan={6} className="p-10 text-center text-ink-faint"><div className="text-2xl mb-2">👥</div>لا يوجد متسابقون مقبولون بعد</td></tr>
              )}
            </tbody>
          </table>
        )}
