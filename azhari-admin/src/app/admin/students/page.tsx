  ...
  return (
    <div className="p-6 md:p-8 space-y-5">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-green-deep">إدارة المتسابقين</h1>
              ...
              {students.map((s) => (
                <tr key={s.id} className="border-b">
                  ...
                  <td className="p-3 space-x-2 space-x-reverse whitespace-nowrap">
                    ...
                    <button onClick={() => setApproval(s.id, "APPROVED")} className="text-green-600 underline text-xs">قبول</button>
                    <button onClick={() => {
                      const reason = prompt("سبب الرفض:");
                      if (reason) setApproval(s.id, "REJECTED", reason);
                    }} className="text-red-600 underline text-xs">رفض</button>
                    <button onClick={() => remove(s.id)} className="text-gray-500 underline text-xs">حذف</button>
                  </td>
                </tr>
              ))}
              {students.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-gray-400">لا يوجد متسابقون مطابقون</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm font-bold text-green-deep pt-2">موعد الامتحان</p>
